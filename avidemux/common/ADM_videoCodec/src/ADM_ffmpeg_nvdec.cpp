...
static AVCodecID cuToLavCid(cudaVideoCodec codec) {
  switch (codec) {
#define MATCH(x, y)                                                            \
  case cudaVideoCodec_##x:                                                     \
    return AV_CODEC_ID_##y;
...
extern "C" {
static enum AVPixelFormat ADM_nvDecGetFormat(struct AVCodecContext *avctx,
  ...
  for (i = 0; fmt[i] != AV_PIX_FMT_NONE; i++) {
    ...
    if (c != AV_PIX_FMT_CUDA)
      continue;
#define FMT_V_CHECK(x)                                                         \
  case AV_CODEC_ID_##x:                                                        \
    outPix = AV_PIX_FMT_CUDA;                                                  \
    id = avctx->codec_id;                                                      \
    break;
...
#undef FMT_V_CHECK
  ...
  hwDevRef = NULL;
  int err =
      av_hwdevice_ctx_create(&hwDevRef, AV_HWDEVICE_TYPE_CUDA, NULL, NULL, 0);
  if (err < 0) {
...
/**
    \fn ctor
*/
decoderFFnvDec::decoderFFnvDec(struct AVCodecContext *avctx, decoderFF *parent)
...
decoderFFnvDec::~decoderFFnvDec() {
  ...
  // hw device context will be uninited and freed when AVCodecContext gets
  // closed
...
bool decoderFFnvDec::uncompress(ADMCompressedImage *in, ADMImage *out) {
  if (!_parent->getDrainingState() &&
      !in->dataLength) // Null frame, silently skipped
  {
    out->_noPicture = 1;
...
bool decoderFFnvDec::readBackBuffer(AVFrame *decodedFrame,
                                    ADMCompressedImage *in, ADMImage *out) {
  AVFrame *copy = NULL;
...
bool decoderFFnvDec::downloadFromRef(ADM_nvDecRef *render, ADMImage *image) {
  ...
  default:
    printf(
        "[decoderFFnvDec::downloadFromRef] Unhandled pixel format: %d (%s)\n",
  ...
  if (!converter)
    converter = new ADMColorScalerSimple(ref._width, ref._height, from,
                                         ADM_PIXFRMT_YV12);
...
public:
  ...
  virtual bool canSupportThis(struct AVCodecContext *avctx,
                              const enum AVPixelFormat *fmt,
                              enum AVPixelFormat &outputFormat);
  virtual ADM_acceleratedDecoderFF *spawn(struct AVCodecContext *avctx,
                                          const enum AVPixelFormat *fmt);
 ...
 *
 */
ADM_hwAccelEntryNvDec::ADM_hwAccelEntryNvDec() { name = "NVDEC"; }
/**
 *
 * @param avctx
 * @param fmt
 ...
 * @return
 */
bool ADM_hwAccelEntryNvDec::canSupportThis(struct AVCodecContext *avctx,
                                           const enum AVPixelFormat *fmt,
                                           enum AVPixelFormat &outputFormat) {
  bool enabled = true;
...
ADM_acceleratedDecoderFF *
ADM_hwAccelEntryNvDec::spawn(struct AVCodecContext *avctx,
                             const enum AVPixelFormat *fmt) {
...
static void printNvDecProbeResults(void) {
  ...
  for (int i = 0; i < listOfNvDecProfiles.size(); i++) {
    admNvDecProfile *p = listOfNvDecProfiles[i];
    ADM_assert(p);
    printf("\t%s\t%d bits\t%s\n", codecDesc(p->cid), p->bitDepth,
           chromaFormatDesc(p->cfm));
  }
  printf("\n");
}
...
static bool checkNvDec(void) {
  ...
  if (!cudaFunc->cuInit || !cudaFunc->cuDeviceGet || !cudaFunc->cuCtxCreate ||
      !cudaFunc->cuCtxDestroy || !cudaFunc->cuCtxPushCurrent ||
      !cudaFunc->cuCtxPopCurrent) {
...
#define CHECK_CAPS(codec, chroma, bit_depth, fatal)                            \
  ...
  ADM_info("Checking " #codec " %d bits %s\n", bit_depth,                      \
           chromaFormatDesc(caps.eChromaFormat));                              \
  ...
  if (r < 0) {                                                                 \
    ...
    if (caps.bIsSupported) {                                                   \
      ...
      if (fatal) {                                                             \
        ADM_warning("Check for " #codec                                        \
                    " has failed, skipping further checks.\n");                \
        goto fail;                                                             \
      }                                                                        \
 ...
 */
bool nvDecProbe(void) { return checkNvDec(); }
